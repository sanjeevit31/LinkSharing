...
<a href="#show-topic" class="skip" tabindex="-1"><g:message code="default.link.skip.label"
...
<div id="show-topic" class="content scaffold-show" role="main">
    ...
    <ol class="property-list topic">
        ...
        <g:if test="${topicInstance?.name}">
            <li class="fieldcontain">
                ...
                <span class="property-value" aria-labelledby="name-label"><g:fieldValue bean="${topicInstance}"
                                                                                        field="name"/></span>
        ...
        <g:if test="${topicInstance?.summery}">
            <li class="fieldcontain">
                <span id="summery-label" class="property-label"><g:message code="topic.summery.label"
                                                                           default="Summery"/></span>

                <span class="property-value" aria-labelledby="summery-label"><g:fieldValue bean="${topicInstance}"
                                                                                           field="summery"/></span>
        ...
        <g:if test="${topicInstance?.user}">
            <li class="fieldcontain">
                <span id="user-label" class="property-label"><g:message code="topic.user.label"
                                                                        default="New Users"/></span>

                <span class="property-value" aria-labelledby="user-label">
                    <g:if test="${flag}">
                        <g:link controller="user" action="show" id="${topicInstance?.user?.id}">
                            ${topicInstance?.user?.encodeAsHTML()}
                        </g:link>
                    </g:if>
                    <g:else>
                        ${topicInstance?.user?.encodeAsHTML()}
        ...
        <g:if test="${topicInstance?.visibility}">
            <li class="fieldcontain">
                <span id="visibility-label" class="property-label"><g:message code="topic.visibility.label"
                                                                              default="Visibility"/></span>

                <span class="property-value" aria-labelledby="visibility-label"><g:fieldValue bean="${topicInstance}"
    ...
    <center>
        <span id="visibility1-label" class="property-label">
            <g:remoteLink controller="resource" action="topicsResource" params="${['topicid': topicInstance.id]}"
                          update="seeResources">
    ...
    <g:form url="[resource: topicInstance, action: 'delete']" method="DELETE">
        <fieldset class="${TopicDeleteEditFlag}">
            <g:link class="edit" action="edit" resource="${topicInstance}"><g:message code="default.button.edit.label"
                                                                                      default="Edit"/></g:link>
            ...
            <g:if test="${topicSubscribeFlag}">
                ...
                <g:link controller="subscribed" action="create" params="[topicid: topicInstance.id]"><input
                        type='button' name="Subscribe" value="Subscribe"/></g:link>
            </g:if>
            <g:else>
