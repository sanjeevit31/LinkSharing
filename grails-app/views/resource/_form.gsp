...
<div class="fieldcontain ${hasErrors(bean: resourceInstance, field: 'new', 'error')} required">
    ...
    <g:textField id="new1" name="new.id1" value="${User.get(session.userId)}" readonly=""/>
    <g:hiddenField id="new" name="new.id" value="${session.userId}" readonly=""/>
...
<div class="fieldcontain ${hasErrors(bean: resourceInstance, field: 'topic', 'error')} required">
    ...
    <g:select id="topic" name="topic.id" from="${com.linksharing.Topic.list()}" optionKey="id" required=""
              value="${resourceInstance?.topic?.id}" class="many-to-one"/>
